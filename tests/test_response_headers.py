...
class TestResponseHeaderUtils(TestCase):
    ...
    def test_guess_next_update(self) -> None:
        ...
        dt = datetime(year=2018, month=9, day=11, hour=19, minute=59, tzinfo=tz)
        next_publish_dt = guess_next_update(dt)
        expected_publish_dt = datetime(year=2018, month=9, day=12, hour=0, minute=0, tzinfo=tzutc())
        self.assertEquals(next_publish_dt, expected_publish_dt)

        dt = datetime(year=2018, month=9, day=11, hour=20, minute=1, tzinfo=tzutc())
        next_publish_dt = guess_next_update(dt)
        expected_publish_dt = datetime(year=2018, month=9, day=12, hour=0, minute=0, tzinfo=tzutc())
        self.assertEquals(next_publish_dt, expected_publish_dt)
        ...
        next_publish_dt = guess_next_update(dt)
        expected_publish_dt = datetime(year=2018, month=9, day=13, hour=0, minute=0, tzinfo=tzutc())
        self.assertEquals(next_publish_dt, expected_publish_dt)
        ...
        dt = datetime(year=2018, month=9, day=14, hour=20, minute=1, tzinfo=tz)
        next_publish_dt = guess_next_update(dt)
        expected_publish_dt = datetime(year=2018, month=9, day=17, hour=00, minute=0, tzinfo=tzutc())
        self.assertEquals(next_publish_dt, expected_publish_dt)
