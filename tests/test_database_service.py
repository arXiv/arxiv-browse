...
from tests import execute_sql_files, grep_f_count, path_of_for_test
...
class TestBrowseDatabaseService(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        inst_cornell_ip_exclude = database.models.MemberInstitutionIP(
            id=2,
            sid=inst_cornell.id,
    ...
    def test_get_institution_returns_an_id_and_label(self) -> None:
        ...
        inst = TestBrowseDatabaseService.database_service.get_institution("128.84.0.0")
        self.assertEqual(
            inst.get("id"), 1, "Institution ID returned for IP at end of range"
        )
        self.assertEqual(
            inst.get("label"),
            "Cornell University",
        ...
        self.assertIsNone(label, "No institution label returned for non-matching IP")
        label = TestBrowseDatabaseService.database_service.get_institution(
            "128.84.10.1"
        )
        self.assertIsNone(label, "No institution label returned for excluded IP")
        ...
        with self.assertRaises(ValueError) as context:
            TestBrowseDatabaseService.database_service.get_institution("notanip")

        self.assertIn(
            "does not appear to be an IPv4 or IPv6 address", str(context.exception)
        )
    ...
    def test_all_trackback_pings(self) -> None:
        ...
        count_from_db: int = TestBrowseDatabaseService.database_service.count_all_trackback_pings()
        count_from_db_list: int = TestBrowseDatabaseService.database_service.get_all_trackback_pings().__len__()
        ...
        if count_from_file is not None:
            ...
            self.assertIsNotNone(count_from_file, "count of trackback pings is defined")
    ...
    def test_trackback_pings(self) -> None:
        ...
        count_from_db: int = TestBrowseDatabaseService.database_service.count_trackback_pings(
            test_paper_id
        )
        count_from_db_list: int = TestBrowseDatabaseService.database_service.get_paper_trackback_pings(
            test_paper_id
        ).__len__()
        self.assertEqual(
            count_from_db,
    ...
    def test_recent_trackback_pings(self) -> None:
        ...
        tbs: List = TestBrowseDatabaseService.database_service.get_recent_trackback_pings(
            max_trackbacks=-1
        )
        ...
        for tb in tbs:
            ...
            self.assertIsInstance(
                Identifier(arxiv_id=tb[1]), Identifier, "Value looks like an Identifier"
            )
    ...
    def test_get_datacite_doi(self) -> None:
        """Test whether paper has a DataCite DOI."""
        test_paper_id = "0906.3421"
        self.assertEqual(
            TestBrowseDatabaseService.database_service.get_datacite_doi(
                test_paper_id, account="test"
            ),
            f"10.1234/arXiv.{test_paper_id}",
        )
        self.assertIsNone(
            TestBrowseDatabaseService.database_service.get_datacite_doi(
                test_paper_id, account="prod"
            ),
        )
    ...
    def test_get_dblp_authors(self) -> None:
        ...
        self.assertListEqual(
            TestBrowseDatabaseService.database_service.get_dblp_authors(test_paper_id),
            [
    ...
    def test_error_conditions(self, mock_query) -> None:
        ...
        self.assertListEqual(
            TestBrowseDatabaseService.database_service.get_paper_trackback_pings(
                "0704.0361"
            ),
            [],
        )
        self.assertEqual(
            TestBrowseDatabaseService.database_service.count_trackback_pings(
                "0704.0361"
            ),
            0,
        )
        ...
        self.assertRaises(
            SQLAlchemyError,
            TestBrowseDatabaseService.database_service.get_institution,
            "10.0.0.1",
        )
        self.assertRaises(
            SQLAlchemyError,
        ...
        self.assertRaises(
            SQLAlchemyError,
            TestBrowseDatabaseService.database_service.count_all_trackback_pings,
        )
        self.assertRaises(
            SQLAlchemyError,
            TestBrowseDatabaseService.database_service.has_sciencewise_ping,
    ...
    def test_sequential_id(self) -> None:
        ...
        self.assertEqual(
            get_sequential_id(Identifier("0906.3421"), is_next=True), "0906.4150"
        )
        self.assertTrue(
            get_sequential_id(Identifier("0906.9150"), is_next=True).startswith("0907")
        )
        self.assertEqual(
            get_sequential_id(Identifier("0906.3421"), is_next=False), "0906.3336"
        )
        self.assertTrue(
            get_sequential_id(Identifier("0907.2020"), is_next=False).startswith("0906")
        )
