...
def test_unavailable(client_with_test_fs):
    ...
    expected_keys=["paper-unavailable", "paper-id-1208.9999", "paper-id-1208.9999-current"]
    ...
    keys= " "+headers["Surrogate-Key"]+" "
    expected_keys=["paper-unavailable", "paper-id-1208.9999", "paper-id-1208.9999v1"]
    assert all(" "+item+" " in keys for item in expected_keys)
    ...
    assert headers["Surrogate-Control"]== "max-age=604800"
    keys= " "+headers["Surrogate-Key"]+" "
    expected_keys=["paper-unavailable", "paper-id-1208.9999", "paper-id-1208.9999v3"]
    assert all(" "+item+" " in keys for item in expected_keys)
