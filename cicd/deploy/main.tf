...
resource "google_cloud_run_v2_service" "arxiv_browse" {
  ...
  labels = {
    "arxiv-system"    = "dissemination"
    "arxiv-subsystem" = "dis-browse"
  }
  ...
  template {
    ...
    containers {
      ...
      env {
        name  = "LATEXML_BUCKET"
        value = var.latexml_bucket
      }

      env {
        name  = "BROWSE_MINIMAL_BANNER_ENABLED"
        value = var.browse_minimal_banner_enabled
      }

      env {
        name  = "BROWSE_USER_BANNER_ENABLED"
        value = var.browse_user_banner_enabled
      }

      env {
        name  = "BROWSE_SPECIAL_MESSAGE_ENABLED"
        value = var.browse_special_message_enabled
      }
