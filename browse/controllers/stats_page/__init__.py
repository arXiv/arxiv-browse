...
def get_hourly_stats_csv(requested_date_str: Optional[str] = None) -> Response:
    ...
    for r in rows:
        hour_dt: str = datetime(
            r.ymd.year, r.ymd.month, r.ymd.day, hour=r.hour
...
def get_monthly_submissions_page() -> Response:
    ...
    try:
        num_submissions, historical_delta = get_monthly_submission_count()
    ...
    try:
        ...
        if rows and rows[-1].ym < current_date:
            ...
            if this_month_count > 0:
                csv_data = (
                    csv_data
                    + f"{current_date.strftime('%Y-%m')},{this_month_count},0\n"
                )
        return (
            {"csv": csv_head + csv_data},
            status.OK,
