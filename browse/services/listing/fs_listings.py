...
class FsListingFilesService(ListingService):
    ...
    def __init__(self, params=None):
        if params != None and params.get('listing_files'):
            self.listing_files_root = params.get('listing_files')
        else:
            self.listing_files_root = app_config.get('DOCUMENT_LISTING_PATH')

    def _is_rule(self, line: str, type: str) -> Tuple[str, str]:
    ...
    def _get_updates_from_list_file(self, listingFilePath: str,
                                    listingType: str, listingFilter: str):
        ...
        import codecs
        with codecs.open(listingFilePath, 'r', encoding='utf-8',
                         errors='ignore') as lf:
            lines = lf.readlines()
    ...
    def _generate_listing_path(self, listingType: str, archiveOrCategory: str,
        ...
        if archiveOrCategory in taxonomy.ARCHIVES:
            ...
            res = re.match(f'([^\\.]*)(?P<suffix>\\.[^\\.]*)$', archiveOrCategory)
            if res:
