...
def parse_new_listing_file(year:int, month: int, listingFilePath: APath,
                           listingFilter: str='')\
...
# def lines(file: APath) -> Generator[str, None, None]:
#     with file.open('rb') as fh:
#         sr = codecs.StreamReader(fh, errors='ignore')
#         line = sr.readline()
#         while(line):
#             yield(line)
#             line = sr.readline()

# def parse_for_count(isting_lines: Iterator[str], listingType: ListingTypes) -> dict:
#     """Just gets if it is a cross or not, and the categories.

#     This should work for pastweek, month and new file formats."""
#     pass

# def parse_no_abs(listing_lines: Iterator[str], listingType: ListingTypes) -> dict:
#     """Parse everything it can find assuming there is no abs.

#     This is intended for the pastweek and month listing file formats."""
#     pass


# def parse_with_abs(lines: Generator[str, None, None], listingType: ListingTypes) -> dict:
#     """Parse and assume there is an abs.

#     This is intended for listing new files."""
#     extras = {}
...
#             res = re.match(r'(.*)\s+to\s+(.*)$', date_short_from_to)
#             if res:
#                 extras['submit_start_date'] = datetime.strptime(res.group(1), '%a %d %b %y ')
#                 extras['submit_end_date'] = datetime.strptime(res.group(2), '%a %d %b %y')

#     # line should contain the \\ just before the first listing item


# def parse_new_item(lines: Generator[str,None,None]) -> ListingItem:
#     """Lines must be a generator and the next line returned should be either the \\ just before
#     the listing item or the first line of the listing item.

#     The `lines` generator will be consumed up to the \\ line after the abstract.

#     If goes too far and  gets to a dash line it will raise a ValueError.
#     """
