...
def format(arxiv_id: str, archive: Optional[str] = None) -> Response:
    ...
    redirect_url = check_supplied_identifier(arxiv_identifier, "dissemination.format")
    if redirect_url:
        return redirect_url  # type: ignore

    abs_meta = get_doc_service().get_abs(arxiv_id)
    data = {"arxiv_id": arxiv_identifier.id, "arxiv_idv": arxiv_identifier.idv, "abs_meta": abs_meta,
            "encrypted": abs_meta.get_requested_version().source_flag.source_encrypted}

    formats = data["formats"] = abs_meta.get_requested_version().formats()

    if abs_meta.get_requested_version().source_format == "pdftex":  # ARXIVCE-3376 no ps for pdflatex
        formats = [fmt for fmt in formats if fmt != "ps"]
